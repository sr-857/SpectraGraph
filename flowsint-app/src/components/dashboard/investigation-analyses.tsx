...
function AnalysisCard({ analysis, investigationId }: { analysis: any; investigationId: string }) {
  return (
    <Link
...
function InvestigationAnalysesSkeleton() {
  return (
...
export function InvestigationAnalyses({ investigationId }: InvestigationAnalysesProps) {
  const { data: analyses, isLoading } = useQuery({
    queryKey: ['analyses', 'investigation', investigationId],
  ...
  if (isLoading) {
    return <InvestigationAnalysesSkeleton />
  }
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 cq-sm:grid-cols-2 cq-md:grid-cols-3 cq-lg:grid-cols-4 cq-xl:grid-cols-5 gap-4">
        {analyses.slice(0, 8).map((analysis: any) => (
          <AnalysisCard key={analysis.id} analysis={analysis} investigationId={investigationId} />
        ))}
      </div>
    </div>
  )
